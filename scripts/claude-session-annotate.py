...
from session_annotation.claude import ClaudeSessionAnnotator, write_csv  # noqa: E402


def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Annotate a Claude Code session JSONL transcript.")
    parser.add_argument("path", type=Path, help="Path to a Claude Code session .jsonl file.")
    parser.add_argument("--jsonl", action="store_true", help="Print one annotation JSON object per line.")
    parser.add_argument("--csv", type=Path, help="Write flattened annotation CSV to this path.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = ClaudeSessionAnnotator(include_text=args.include_text).annotate_file(args.path)
    ...
    if args.jsonl:
        ...
        for diagnostic in report.diagnostics:
            print(json.dumps({"category": "diagnostic", "kind": "parse_diagnostic", **diagnostic.__dict__}, default=str, sort_keys=True))
    else:
