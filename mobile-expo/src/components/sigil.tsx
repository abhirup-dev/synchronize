...
function harnessOf(tool?: string): 'claude' | 'codex' | 'pi' | 'other' {
  const t = (tool ?? '').toLowerCase();
  if (t.includes('claude')) return 'claude';
  if (t.includes('codex') || t.includes('openai') || t.includes('gpt')) return 'codex';
  if (t === 'pi' || t.includes('inflection')) return 'pi';
  return 'other';
}

function Mark({ harness, size, color }: { harness: string; size: number; color: string }) {
  if (harness === 'claude')
    return (
  ...
  // π letterform (fallback in the reference until lobe-icons load)
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24">
      <Line x1="4" y1="7" x2="20" y2="7" stroke={color} strokeWidth={2.2} strokeLinecap="round" />
      <Line x1="8" y1="7" x2="8" y2="19" stroke={color} strokeWidth={2.2} strokeLinecap="round" />
      <Line x1="16" y1="7" x2="16" y2="19" stroke={color} strokeWidth={2.2} strokeLinecap="round" />
    </Svg>
  );
}
...
export function SigilChip({
  ...
  return (
    ...
    >
      {harness === 'other' ? (
        <Text
          style={{ color, fontFamily: 'JetBrainsMono_700Bold', fontSize: size * 0.34 }}
        >
          {initials(name ?? id)}
        </Text>
      ) : (
        <Mark harness={harness} size={Math.round(size * 0.6)} color={color} />
      )}
    </View>
  );
}
