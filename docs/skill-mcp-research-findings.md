...
- **Disposition:** `SESSION-FIX` norm (fold into `sync-b8p`, tied to F14) + `BD` (routing tweak, weigh vs P2/awareness). Net: solve noise with **ack + a distinct-contribution norm**, NOT a blanket silence rule that would kill the divergent-panel value.
