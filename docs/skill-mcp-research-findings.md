...
- **Disposition:**
  ...
  - `BD` (P2, daemon/UX): the send/reply response should **echo the resolved `thread_root` it normalized to** (e.g. `posted to thread 196`), so an agent (and the `session_stub`) immediately notices a reply landed in an unexpected thread. Turns a silent misroute into a visible one. Ties F9 (`hint`) + P3 (`session_stub`) + F19's full pattern.
