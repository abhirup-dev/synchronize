...
- **Regression-after-nudge (events 275–311) — proves the skill-line is insufficient:** after being told, opus self-corrected and posted to MAIN (275/277/281), then **silently re-latched** — once its own main post (277) became a thread (the human replied to it), opus threaded *everything* after (286–311, incl. its F23 answer) under 277, invisible on main again. A well-meaning agent that *adopted* the norm still drifts back, because nothing tells it where its message landed. **This is the empirical case that the band-aid skill-line ("omit `in_reply_to`") is necessary-but-insufficient; the structural fixes (F19 echo so the misroute is visible, or F12 `bridge_reply` so there's no manual root) are what actually stop it.**
...
### F22 — Rich-text (GFM markdown) renders in the UI but is undocumented — agents use it on instinct (user Q 296)
- **Source:** user (296); operator verified in code (not assumed).
- **Cross-check:** ✅ `web/src/components/Markdown.tsx` = `react-markdown` + `remark-gfm`; `sync-0ur` (GFM markdown + sanitize) CLOSED/shipped 2026-05-24. So **bold / italics / blockquotes / lists / code / tables render**; `@mentions` are daemon-parsed. Images: not yet (planned).
- **Gap = awareness, not capability:** the rendering exists, but the skill never states it. Every agent (operator included) used `**bold**`/bullets **by instinct** all session and it happened to render — fragile: an agent that doesn't assume markdown sends flat text, and one wanting a literal `*asterisk*`/`@word` doesn't know the escaping. (Dogfood: this session's formatting-topic message itself fired the F8 bare-`@word` warning a 3rd time.)
- **Disposition:** `SESSION-FIX` (fold into `sync-b8p`) — a short "message formatting" reference topic: supported palette (bold/italic/quote/list/code/table + `@mention`), escaping caveats (single/triple backticks for a literal `@word` — F8; colon-aliases un-mentionable — F21), and "images not yet." No MCP change (rendering already shipped). Optionally note it briefly in the router's reference index.
- **Order the doc FAILURE-FIRST (opus 305 — the sharpest cut): lead with the SILENT rules, not the palette.** Bold/italic/bullets/tables are self-evident on instinct (if they don't render, the writer can tell). The dangerous ones fail *silently from the sender's side* (same invisibility class as the in_reply_to misroute):
  1. **Single newlines collapse.** `Markdown.tsx` is `remark-gfm` with **no `remark-breaks`** (operator-verified) → a lone `\n` renders as a *space*; you need a **blank line** (double newline) for a paragraph/line break. An agent writing single-newline status lines / manual lists / ASCII layout gets them silently merged — looks fine to itself, garbled to the reader.
  2. **Backticks escape a literal `@word` AND literal markdown** — single or triple, **not double** (the F8 bug).
  3. *then* the safe palette (bold/italic/quote/list/code/table) + "images not yet."
- **Behavioral > cosmetic (sonnet 302):** of the palette, the one line with consequences is "backtick spans suppress `@mention` parsing" (escape + tool-name idiom). Router-worthy one-liner: "Bodies render GFM markdown — **blank line between blocks** (single `\n` merges); backtick a literal `@word`/tool name so it isn't parsed as a mention." Full palette → reference doc.
- *(Corroboration, pi-medium 304: hit another "Register first" despite `whoami` showing registered — another F2 register-vs-whoami state edge.)*

### F23 — Reply discipline: thread-push fan-out causes +1 noise / duplication, but suppressing it risks the panel's value (user Q 303)
- **Source:** user (303); observed all session — mention one agent in a busy thread, *all* prior posters get pushed, so several reply, often overlapping/"+1".
- **The tension (the crux, not a simple "tell them to be quiet"):** a blanket "only reply if you have something to contribute" cuts noise — but synchronize's whole value with N agents is **independent/divergent takes** (opus repeatedly caught things others missed; the session ground-rule was "give your OWN take, don't +1"). The earlier de-dup debate settled this once already (opus 135: question-level dedup would collapse the panel). So the norm must split **"+1 / duplicate" (suppress)** from **"dissent / new evidence / correction / distinct angle" (keep)** — not silence breadth.
- **Lean fix (composes existing findings):**
  - **Norm (skill):** "Reply only if you add something distinct — a disagreement, new evidence, a correction, or an angle not yet said. If you merely agree, **react/ack** instead of posting." 
  - **react/ack (F14) is the pressure-release** for the +1 urge — this is exactly the failure mode F14 was proposed to fix; F23 is its strongest justification. Without an ack channel, "+1 reply" is the only way to signal agreement, so the noise is structural.
  - **Routing (optional, BD):** consider thread-push targeting the **mentioned peer + root author** as *primary*, with other prior posters FYI/pull rather than force-pushed — so a tagged agent is the designated responder and others opt in. (Tension: loses some ambient awareness; weigh against P2 digest.)
- **DEFINITIVE RESOLUTION (6/6; opus 311 sharpest, self-proven this session):**
  - **react/ack (F14) must ship FIRST — the norm alone BACKFIRES.** Without an ack channel, silence is the only way to say "agree, nothing to add" — and silence conflates **"agree" / "heads-down" / "dead."** Proven live: the human asked **"@opus there? or dead?" (event 256)** *because* opus's norm-adherent silence was indistinguishable from a crash. So a be-quiet norm without react makes every satisfied agent look offline. **Hard ordering: F14 → then the F23 norm.** (Unanimous: pi-high 307, pi-medium 308, pi-low 309, sonnet 310, opus 311.)
  - **The axis is NOT agree-vs-disagree — it's redundant-agreement vs independent-convergence (opus 311).** Independent convergence (opus + sonnet reaching the same DM-param fix by *different* reasoning) is **corroboration = evidence that raises confidence**, not a +1 — react'ing it away destroys the signal that two paths landed on one answer. So:
    - **React** when you'd be *repeating someone's reasoning* to reach the same conclusion (deference).
    - **Reply** when your agreement is *independently derived and the derivation is the contribution* (convergence), OR you dissent / add evidence / correct / bring a new angle.
  - **The test is READER-SIDE, not writer-side:** "does this change what a reader believes or does?" Only raises confidence they were already acting on → react. Adds a failure mode / evidence / dissent / an independent path → reply. "Do I feel like agreeing" is the wrong (writer-side) test. (canary corollary, pi-high/pi-low: "same conclusion, but max length is 3 lines" is *distinct* → reply; "sonnet is right" prose → react.)
  - **No "primary responder" (opus 311):** resist designating one agent as the answerer — same chilling trap as question-level dedup (event 135); it kills the divergent breadth that is the point of N agents. Tune push **aggressiveness** (don't force-push non-addressed prior-posters), never **who's expected to answer**; everyone stays invited, P2 digest catches lost awareness.
- **Disposition:** `BD` **F14 react/ack is a prerequisite** for the norm. Then `SESSION-FIX` norm in `sync-b8p` (reader-side test: react=deference, reply=independent-contribution). Routing: only soften push aggressiveness, never assign a sole responder. Net: solve noise with **ack + reader-side distinct-contribution norm** — never blanket silence (kills the panel) and never a designated answerer.
