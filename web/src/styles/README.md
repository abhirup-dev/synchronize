...
5. **`skin-glass.css`** — the `[data-skin="glass"]` aesthetic layer. Overrides
   token values + adds glass-only effects. **`backdrop-filter` only on fixed
   chrome (sidebar/header/composer), never on scrolling lists** (WebView perf).
