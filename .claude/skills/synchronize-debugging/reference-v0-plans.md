...
| `docs/plans/web-theme-token-system-v2.md` | 808 | Web theme token system v2 for moving agent and room identity colors into curated theme palettes, adding a typed theme contract, migrating color persistence, reducing component-specific CSS theme patches, and verifying multi-theme Storybook/live behavior (sync-l1az). |
...
| `docs/plans/web-multi-tab-popout-v0.md` | 182 | Multi-tab popout + cross-tab sync v0: `/web/r/:roomId` deep link and `?view=pane` standalone room/thread surface, open-in-new-tab affordances, server-side per-peer composer drafts (SQLite `drafts` table, `/web/drafts` routes, new `drafts` SSE domain, composer echo suppression), and peer_id surfaced in the web session response. Server-authoritative sync (no BroadcastChannel/SharedWorker); URL contract doubles as the Tauri desktop in-app-tab integration surface. (epic sync-ah0u; sync-5r0s popout, sync-qvkn drafts, sync-ul74 peer_id). |
...
| `docs/plans/launch-flow-architecture.md` | ~300 | Predictable production + worktree launch architecture (branch `feat/dx-foundations`). Separates production daemon, production bundle, isolated dev daemon, and worktree UI dev server, with the launcher holding no daemon lifecycle authority. Specifies: a typed non-mutating `DaemonProbe` replacing the boolean `client.ts:157` health check that makes `ensureDaemon` spawn a second daemon onto a hung one; **Bun HTML entrypoints** so one `index.html` serves Bun and Vite with no placeholder substitution (hashed names verified against the immutable-cache regex at `server.ts:1419`); Capacitor removal, making the asset base the literal `/web/`; the dev config named `vite.dev.config.ts` because `@storybook/react-vite` auto-merges `vite.config.ts`; `base: "/web/"` in all environments, since serving the document at `/` lets client routes reach the daemon SPA fallback (`server.ts:1394`) and return the production bundle; and forwarding as a client-route pass-through list with the daemon as default, because the app calls 14 route families and an allow-list fails dev-only and silently. (epics A/B/C) |
| `docs/plans/coupling-leaks.md` | ~200 | Address and runtime decoupling (branch `feat/dx-foundations`) — the prerequisite for the routing contract. Frames ORIGIN/RUNTIME as deployment axes and BASE/ROUTE as contract layers, with the governing rule that ROUTE is expressible without knowing the other three. D1: `deeplinks.ts:10` holds `/web/` inside the route parser as a regex literal, so extract a pure `web/src/routing/address.ts` owning one `BASE` constant. D2: `daemon.ts:425` infers RUNTIME from `window.location.origin`; the present-but-unpassed `opts.baseUrl` is what lets a worktree UI read the production runtime. Behaviour-preserving — no user-visible URL changes. (epic D) |
| `docs/plans/routing-and-address-model.md` | ~340 | The address contract and TanStack Router adoption (branch `feat/routing-contract`, stacked on `feat/dx-foundations`, which must land first). Grammar has three categories: ADDRESSES (`/web/g/:publicId`, `/web/d/:peerId`, `/web/t/:eventId`), RESOLVERS that redirect then `replaceState` (`/web/e/`, `/web/r/`, `/web/g/by-name/`, `?event=`), and MODIFIERS (`?view=pane` pushState, `?focus=` replaceState-only). Addresses must be opaque because autoincrement ids collide across daemon homes by construction, so a link pasted between runtimes opens a different room instead of failing — hence `groups.public_id`; `peer_id` already qualifies (UUIDv4, `daemon/routes/peers.ts:26`). Adopts TanStack Router because the URL is a state container, loading is per-address with cancellation, and addresses nest; the concrete work is replacing the four-variable deep-link choreography at `App.tsx:246-262` with one loader, while `around_event_id` hydration (`daemon.ts:1010-1017`) is retained unchanged. Cache ownership is specified: loaders are gates, `DaemonDataSource` stays the single SSE-fed cache. Nested layout routes carry shell-vs-pane chrome. `@storybook/tanstack-react` is first-party, so Storybook is a framework swap. Also specifies scoped per-surface endpoints to replace monolithic `/web/state` (`daemon/routes/web.ts:26-44`), anchor-based scroll restoration under `react-virtual`, and seven feature-clustered migration groups (G1–G7) with an explicit unchanged list. Makes this grammar canonical over epic sync-ah0u's `/web/r/:roomId`, which becomes a resolver. Supersedes the addressing part of `web-url-deep-links.md`. |
| `docs/plans/glass-skin-revamp-port.md` | ~230 | Port the glass skin revamp (claude.ai/design project 41739566) onto master's unified token system — mono variant only, mesh ambient only, no variant fixtures. Five phases: mono token foundation → expanding-rail control primitives (Rail/RailSegment/RailChip, `--rail-*` tokens) → floating headers/composer + hard-cut masks (NO fades) + minimal surfaces → compact/medium behaviors + settings axes (`data-you`, `data-agentlabel`) → roster model chips, AgentProfile model picker + daemon `setAgentModel`, ArtifactsView, seed plumbing, design-sync sync-back. Key rule: port intent, not the bundle's CSS-override hacks (labels in markup, restructure in TSX, shellLayout traits). Design truth in `ds-bundle/templates/glass-skin-revamp*/` + remote handoffs (epic sync-oiqr; phases sync-oiqr.1–.5, follow-up sync-oiqr.6). |
