...
| `docs/plans/launch-flow-architecture.md` | 382 | Predictable production and worktree launch architecture using explicit daemon ownership, a Vite HMR UI server, T3Code readiness patterns, and Portless worktree URLs (sync-o02e; sync-o02e.1–.4). |
...
| Production/worktree UI launch boundaries | `src/client.ts`, `web/build.ts`, `web/src/data/daemon.ts`, `src/daemon/server.ts`, `Makefile` | `docs/plans/launch-flow-architecture.md` |
