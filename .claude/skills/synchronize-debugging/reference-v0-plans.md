...
| `session-tracker/plan-session-annotation-architecture-v1.md` | 1364 | Future-proof session annotation architecture for pluggable host tools, incremental append, overlays, on-demand cross-machine status, and FTS/Tantivy projections (sync-4v00). | Session annotation lake, host adapter interfaces, incremental append, delayed bookmarks/tags, runtime derivation, federated annotation modes |
