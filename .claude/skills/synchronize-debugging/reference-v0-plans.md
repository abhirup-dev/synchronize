...
| `docs/plans/resumable-archived-sessions.md` | 538 | Archive + faithful context resume of Claude/Pi sessions and groups. Two orthogonal axes (presence/lease vs lifecycle active\|archived; deleted_at=released); archived IS resumable. Faithful resume via `claude --resume <host_session_id>` / `pi --session <id>` reattached through the existing `upsertPeer` resurrection + `host_session_id` correlation + `ENV_PEER_ID`. Explicit + opt-in auto-archive (off the 24h sweeper); AOE reaped on archive (free-the-runtime); resume gated on a real-time liveness probe (not the stored lease); pure `transitionArchive` state machine reusing `launch_work`. Diagram-first; flows A–K. | Epics `sync-g0ur` (foundation), `sync-l2lt` (GC/stop guards: daemon.ts:686/2127/2178 + multi-launch-per-peer), `sync-8rc3` (explicit archive), `sync-jpxm` (resume), `sync-qqef` (auto-archive), `sync-cmw2` (MCP/e2e/web). `peers.lifecycle_state`, `group_members.member_state` + alias-index swap, archive/resume CLI/REST/MCP surface |
