...
- **Thread summaries.** Use `bridge_get_thread_summary` to read a cached LLM summary of a thread. The daemon auto-summarizes cold threads (no activity for ~30 min) in the background; pass `force: true` to recompute now (and optionally `strategy: "all" | "first_k" | "last_k" | "first_last"` with `k`/`first_k`/`last_k`). The response includes `status` (`ready` | `pending` | `disabled`) and a `stale` flag — when disabled, the daemon has no LLM provider configured and there are no summaries to read. Prefer the cached read; only force-regen when the summary is stale or the strategy needs to change.
