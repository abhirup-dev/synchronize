...
test("openDatabase creates durable launch lifecycle tables idempotently", async () => {
  ...
  expect(db.query<{ version: number }, []>("SELECT version FROM schema_migrations WHERE version = 7").get()?.version).toBe(7);
  db.close();

  const reopened = await openDatabase(path);
  expect(reopened.db.query<{ version: number }, []>("SELECT version FROM schema_migrations WHERE version = 7").get()?.version).toBe(7);
  reopened.db.close();
});
